...
const ViewViewVisitListCanceledPageTc = () => {
    ...
    if (idTechnical) {
      handleGetVisitAssign(idTechnical)
        .then((res) => {
          ...
          const visitasCanceladas = datosVisitas.filter(visita => visita.estado === "cancelada");

          setVisits(visitasCanceladas);
          setFilteredVisits(visitasCanceladas);
        })
        .catch((err) => {
  ...
  return (
    <Container $collapsed={collapsed}>
      <BaseHeaderSection
        headerTitle="VISITAS CANCELADAS"
        sectionTitle="Listado de visitas canceladas"
      ...
      <Card>
        <ContainerService>
          {filteredVisits.length === 0 ? (
            <p style={{textAlign: "center", marginTop: "20px"}}>No tienes ninguna visita en estado "Cancelada" por el momento.</p>
          ) : (
...
export default ViewViewVisitListCanceledPageTc;