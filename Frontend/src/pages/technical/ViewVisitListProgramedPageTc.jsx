...
const ViewViewVisitListProgramedPageTc = () => {
    ...
    if (idTechnical) {
      handleGetVisitAssign(idTechnical)
        .then((res) => {
          ...
          const visitasProgramadas = datosVisitas.filter(visita => visita.estado === "programada");

          setVisits(visitasProgramadas);
          setFilteredVisits(visitasProgramadas);
        })
        .catch((err) => {
  ...
  return (
    <Container $collapsed={collapsed}>
      <BaseHeaderSection
        headerTitle="VISITAS PROGRAMADAS"
        sectionTitle="Listado de visitas programadas"
      ...
      <Card>
        <ContainerService>
          {filteredVisits.length === 0 ? (
            <p style={{textAlign: "center", marginTop: "20px"}}>No tienes ninguna visita en estado "Programada" por el momento.</p>
          ) : (
...
export default ViewViewVisitListProgramedPageTc;