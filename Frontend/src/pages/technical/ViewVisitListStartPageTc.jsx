...
const ViewViewVisitListStartPageTc = () => {
    ...
    if (idTechnical) {
      handleGetVisitAssign(idTechnical)
        .then((res) => {
          ...
          const visitasIniciadas = datosVisitas.filter(visita => visita.estado === "iniciada");

          setVisits(visitasIniciadas);
          setFilteredVisits(visitasIniciadas);
        })
        .catch((err) => {
  ...
  return (
    <Container $collapsed={collapsed}>
      <BaseHeaderSection
        headerTitle="VISITAS INICIADAS"
        sectionTitle="Listado de visitas iniciadas"
      ...
      <Card>
        <ContainerService>
          {filteredVisits.length === 0 ? (
            <p style={{textAlign: "center", marginTop: "20px"}}>No tienes ninguna visita en estado "Iniciada" por el momento.</p>
          ) : (
...
export default ViewViewVisitListStartPageTc;