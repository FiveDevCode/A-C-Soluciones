...
export class ServicioController {
    ...
    try {
      ...
      if (servicioExistente) {
        return res.status(400).json({
          success: false, message: 'Ya existe un servicio con ese nombre.'
        });
      }
      const dataServicio = {
        ...req.body, creada_por_fk: req.user.id, id_administrador: req.user.id
      };

      const nuevoServicio = await this.servicioService.crearServicio(dataServicio);

      return res.status(201).json({
        success: true, message: 'Servicio creado correctamente', data: nuevoServicio
      });
  ...
  obtenerServicios = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        success: true, count: servicios.length, data: servicios
      });
  ...
  actualizarServicio = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        success: true, message: 'Servicio actualizado correctamente', data: servicioActualizado
      });
    } catch (error) {
