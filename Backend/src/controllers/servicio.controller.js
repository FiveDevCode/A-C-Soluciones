  ...
  obtenerServiciosAsignados = async (req, res) => {
    try {
      if (req.user.rol !== 'tecnico') {
        ...
        return res.status(403).json({ success: false, message: 'Acceso denegado. Solo los técnicos pueden consultar sus servicios asignados.'});
      }
      ...
      if (visitas.length === 0) {
        ...
        return res.status(200).json({ success: true, message: 'No tienes servicios asignados por el momento.', data: []
        });
      }
      return res.status(200).json({ success: true, message: 'Servicios asignados obtenidos correctamente.', count: visitas.length, data: visitas
      });
    } catch (error) {

      console.error('Error al obtener servicios asignados:', error);

      return res.status(500).json({ success: false, message: 'Error al obtener los servicios asignados.'});
    }
